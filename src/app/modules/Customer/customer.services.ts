...
const getByIdFromDB = async (customerId: string): Promise<TCustomer | null> => {
...
const updateIntoDB = async (
  customerId: string,
  data: TCustomerUpdate
): Promise<TCustomerUpdate> => {
  const existingCustomer = await prisma.customer.findUnique({
    where: { customerId },
  });
...
const deleteFromDB = async (customerId: string): Promise<TCustomer | null> => {
  if (!customerId) {
